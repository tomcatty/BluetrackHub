...
#ifdef DEBUG
            err_code = app_uart_put_string("Service commmand pending\n\r");
            APP_ERROR_CHECK(err_code);
#endif
        }
    }
 ...
 */
static void uart_handler(app_uart_evt_t * p_app_uart_event)
{
    ...
    switch (p_app_uart_event->evt_type)
    {
        case APP_UART_DATA_READY:
            // App does not receive data over UART, but we need to service the FIFO
        ...
        case APP_UART_FIFO_ERROR:
            APP_ERROR_CHECK(p_app_uart_event->data.error_code);
            break;

        case APP_UART_COMMUNICATION_ERROR:
            APP_ERROR_CHECK(p_app_uart_event->data.error_communication);
            break;
        ...
        case APP_UART_DATA:
            // Nothing to do, app does not receive data over UART.
            break;
...
static void uart_init(void)
{
    ...
    uart_comm_params.rx_pin_no = UART_RX;
    uart_comm_params.tx_pin_no = UART_TX;
    uart_comm_params.rts_pin_no = UART_RTS;
    uart_comm_params.cts_pin_no = UART_CTS;
    uart_comm_params.flow_control = APP_UART_FLOW_CONTROL_ENABLED;
    uart_comm_params.use_parity = false;
    uart_comm_params.baud_rate = UART_BAUDRATE_BAUDRATE_Baud38400;

    APP_UART_FIFO_INIT(&uart_comm_params, UART_FIFO_RX_SIZE, UART_FIFO_TX_SIZE, uart_handler, APP_IRQ_PRIORITY_LOW, err_code);
    APP_ERROR_CHECK(err_code);
}
#endif
...
static void gap_params_init(void)
{
    ...
    // Set address to random private non resolvable to ensure iOS detects device correctly after coming out of DFU
    address.addr_type = BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_NON_RESOLVABLE;
    err_code = sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_AUTO, &address);
    APP_ERROR_CHECK(err_code);
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_TIMEOUT:
            ...
            {
                // Simply commence advertising again
                advertising_start();
            }
            break;
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    ble_bluetrack_on_ble_evt(&m_bluetrack, p_ble_evt);
}
...
static void ble_stack_init(void)
{
    ...
    // Enable concurrent radio activity and CPU execution
    cpu_blocking_enabled.common_opt.radio_cpu_mutex.enable = 0;
    err_code = sd_ble_opt_set(BLE_COMMON_OPT_RADIO_CPU_MUTEX, &cpu_blocking_enabled);
    APP_ERROR_CHECK(err_code);
}
